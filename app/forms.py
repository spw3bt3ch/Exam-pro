...
CLASS_CHOICES = [(f"Primary {i}", f"Primary {i}") for i in range(1, 7)] \
    ...
    + [(f"SS {i}", f"SS {i}") for i in range(1, 4)]  # Allow all SS classes for teacher-created subjects
ALL_CLASS_CHOICES = [("", "All classes")] + CLASS_CHOICES

# Full class choices for student registration (includes SS2 and SS3)
FULL_CLASS_CHOICES = [(f"Primary {i}", f"Primary {i}") for i in range(1, 7)] \
    + [(f"JSS {i}", f"JSS {i}") for i in range(1, 4)] \
    + [(f"SS {i}", f"SS {i}") for i in range(1, 4)]
FULL_ALL_CLASS_CHOICES = [("", "All classes")] + FULL_CLASS_CHOICES
...
class RegisterForm(FlaskForm):
    ...
    class_name = SelectField("Class", choices=FULL_ALL_CLASS_CHOICES, validators=[Optional()])
